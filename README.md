...
- 采集结果通过 Shadowrocket 日志和通知观察；每次注入都会发送“292 打票成功”提示栏通知。
...
请允许 Shadowrocket 发送系统通知并开启横幅，以便看到以下提醒：

- **292 打票成功**：每次请求写入缓存 state 时通知，显示 state 长度和剩余有效期；表示本地注入完成，不代表上游请求已成功。
- **已采集**：探针或正式响应采集通过校验时通知，显示实际剩余有效期与续期时间。
- **收到 312，请注意**：探针或正式响应返回 312 字符 state 时提醒，默认不会把它作为新的 292 缓存。
- **state 未通过校验**：其他格式、块数或有效期异常。
- **认证或访问失败／请求限流／响应异常／网络异常**：提醒检查账号、网络或稍后重试。
- **探针未采集到 state／探针未完成／本次未注入**：说明采集未成功或当前无有效缓存，请求仍按原逻辑处理。

每次注入及每次异常事件都会尝试通知，不做通知去重。系统通知权限、专注模式等可能影响横幅展示；通知 API 不可用或发送失败不会阻断请求。
