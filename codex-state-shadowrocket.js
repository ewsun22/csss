...
function flowIdentifier(url, headers) {
  ...
  return fingerprint([
    ...
    context.thread,
    context.turn,
    String(getHeader(headers || {}, "x-request-id") || "")
  ].join("\u0000"));
}

function statusOf(response) {
  return Number(response && (response.status || response.statusCode || response.status_code)) || 0;
}

function notify(title, subtitle, body) {
  if (typeof $notification === "undefined" || !$notification ||
      typeof $notification.post !== "function") return;
  try {
...
function handleResponse(options) {
  ...
  const store = readStore();
  cleanFlows(store, now);
  const flowId = flowIdentifier($request.url, requestHeaders);
  const flow = store.flows[flowId];
  if (flowId) delete store.flows[flowId];
  const key = flow ? flow.key : entryKey(requestHeaders, options);
  const current = store.entries[key];
  const responseHeaders = (typeof $response !== "undefined" && $response.headers) || {};
  const responseStatus = statusOf($response);
  const state = extractState(responseHeaders);
  const token = responseStatus === 200 ? acceptState(state, options, now) : undefined;
  const observed = parseState(state);
  updateHistory(store, flowId, {
    responseAt: now,
    responseStatus,
